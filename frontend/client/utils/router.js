...
export class Router {
    ...
    async handleRouteChange() {
        ...
        // Tách phần path và query string
        const [path, queryString] = hash.split('?');

        const routeInfo = this.findRoute(path); // Chỉ truyền path cho findRoute
        ...
        if (routeInfo) {
            ...
            if (typeof Component !== 'function') {
                console.error('Invalid component for route:', path);
                return this.showErrorPage();
            }
            ...
            try {
                await this.currentComponent.render(this.app);
                console.log(`Rendered component for route: ${path}`);
